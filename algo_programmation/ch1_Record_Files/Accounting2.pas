...
Procedure prGestionJournal(Var FJournal:TFJournal;var FPlan:TFPLan);
 ...
 (* ------------------------------------------- prSaisiePiece ------------------------------*)
	...
	begin
	 ClrScr;
        Writeln('Donnez le code du compte :');
        Readln(CodeCompte);
        ClrScr;
        Writeln(CodeCompte);
        Writeln(fnLookUpForAccount(CodeCompte,FPlan));
		If fnLookUpForAccount(CodeCompte,FPlan)<>'Compte n''existe pas. Veillez l''ajouter' Then
		Begin
            While Not EOF(FJournal) Do
            Begin
                Read(FJournal);
